...
pub fn main(init: std.process.Init) !void {
    ...
    _ = try feed_view.layout(gpa, &engine, @intCast(design_w), @intCast(logicalH(W, H)), posts, 0, &dl, null, null, false, 0, null);
    ...
    while (true) {
        ...
        if (win.fb.width != cur_w or win.fb.height != cur_h) {
            ...
            bias = new_bias;
            dl.len = 0;
            _ = feed_view.layout(gpa, &engine, @intCast(design_w), @intCast(logicalH(cur_w, cur_h)), posts, 0, &dl, null, null, false, 0, null) catch break;
