...
// ── THE MESSAGE CONTEXT MENU (CHAT_FEATURES slice 2) ────────────────────────
//
// The spine. There was no press-and-hold ANYWHERE in this app and no right-click
// menu on a message, which is why there was no delete, no reply, no reactions and
// no forward: they are all entries in a menu that did not exist.
//
// It is an OVERLAY, so it obeys the standing checklist that has bitten three times:
// it draws LAST (over everything), it CONSUMES its input (a tap while it is open
...
fn drawChatMenu(
    ...
    // every one of them — a menu whose buttons do nothing, which is exactly the
    // failure the overlay law exists to prevent.
    try emitRegion(gpa, regions, 0, 0, width, @intCast(@max(0, @min(height, std.math.maxInt(u16)))), 0, .chat_menu_dismiss);

    try cardBox(gpa, dl, x + 2, y + 6, chat_menu_w, hh, 12, softA(0x000000, @intCast(@as(u32, a) / 3)));
...
/// The composer's current job: a plain message, an answer to one, or an edit of one.
/// A7.2: cold, one per frame.
pub const ChatComposeCtx = struct {
    /// One line of the message being answered/edited ("" = neither).
    text: []const u8 = "",
    editing: bool = false,
};
...
pub fn layoutChat(
    ...
    /// The thread's game boards, addressed by `BubbleRow.game` — one per game
    /// move, each the board as of that move (only the newest is interactive).
    games: []const chat_view.GameCard,
    ...
    /// What the open conversation's peer knows about it (A1). `waiting` and
    ...
    devices: ChatDevices,
    /// CHAT_FEATURES slice 2: the message context menu. Drawn LAST, over everything,
    /// and it eats every tap that is not one of its own.
    menu: ChatMenu,
    ...
    for (thread, bh, 0..) |b, hh, idx| {
        ...
        if (by + hh > thread_top and by < clip_bot) {
            if (b.kind == .system) {
                ...
                // THE QUOTE COUNTS TOWARD THE WIDTH. A bubble that shrink-wrapped to
                // "yes" while carrying a quoted line three times as long left the quote
                // hanging out over the edge of its own bubble — which is exactly what
                // the first render of this showed.
                const q_w: i32 = if (b.quote.len > 0 and !b.deleted)
                    @intCast(text.measure(e, .regular, b.quote, 12) + 10)
                else
                    0;
                const content_w = @max(one_w, q_w);
                const fits_one = std.mem.indexOfScalar(u8, b.body, '\n') == null and content_w <= bub_max - 2 * pad_x;
                const bw = if (fits_one) content_w + 2 * pad_x else bub_max;
                const bx = if (b.mine) detail_x + detail_w - bw else detail_x;
                const fill: u32 = bubbleFill(accent, b.mine);
                const bub_rad: u8 = if (phone) 17 else 14; // rounder on phone
                // THE BUBBLE IS A TARGET (CHAT_FEATURES slice 2). It never was: a
                // message could not be pressed, so nothing could be done to one. The
                // region carries the STORE index, so what the menu acts on is the
                // message itself and not "the third row from the top", which stops
                // being the same message the moment anything arrives.
                // The ORDINAL in the thread, not the store index: a Region carries a
                // u16, and a long history can exceed that. The shell maps the ordinal
                ...
                if (!is_fly or bfx_group) {
                    ...
                    if (b.quote.len > 0 and !b.deleted) {
                        ...
                        // one clipped line. In a thread where four things are in the
                        // air, "yes" on its own answers nothing at all.
    ...
    // ── THE DELIVERY LINE (A1). A conversation can be alive on your side and
    // absent on theirs: the Welcome that opens it is one shot across a relay
    // whose store is in-memory, and if it is lost, every message after it goes
    // nowhere. It used to say nothing — the thread looked exactly like a
    // working one. Now it says what is true, and the client keeps re-sending
    // the Welcome underneath until the peer's ack retires this strip.
    //
    // A confirmed conversation draws NOTHING: healthy plumbing is silent.
    if (peer.len > 0 and delivery != .confirmed) {
        const strip_h: i32 = 26;
        const by = thread_top;
        try rect(gpa, dl, detail_x, by, detail_w, strip_h, bg, 0);
        try rect(gpa, dl, detail_x, by, detail_w, strip_h, skinHeaderVeil(accent), 0);
    ...
    // accident; one you cannot cancel is a trap.
    if (compose_ctx.text.len > 0) {
        const sh2: i32 = 34;
        const sy2 = comp_y - sh2 - 6;
        try cardBox(gpa, dl, input_x, sy2, input_w, sh2, 9, 0xFF232326);
        try rect(gpa, dl, input_x, sy2, 2, sh2, accent, 1);
        const label: []const u8 = if (compose_ctx.editing) "Editing" else "Replying to";
        const lw2: i32 = @intCast(text.measure(e, .semibold, label, 11));
        _ = try str(gpa, dl, e, .semibold, input_x + 10, sy2 + 14, softA(accent & 0x00FFFFFF, 0xDD), 11, label);
        try strEllipsis(gpa, dl, e, .regular, input_x + 10, sy2 + 28, muted, 12, compose_ctx.text, input_w - lw2 - 50);
        // The way out.
        const bx2 = input_x + input_w - 26;
        _ = try str(gpa, dl, e, .semibold, bx2, sy2 + 22, muted, 14, "\u{00D7}");
    ...
    // THE MENU, LAST. Over the sheets, over the cards, over everything — and it eats
...
test "messages screen: master-detail chat surface (list, thread, composer)" {
    ...
    // no reply and no reactions — they are all entries in a menu that can only be
    // summoned FROM a message.
