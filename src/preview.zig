...
pub fn main(init: std.process.Init) !void {
    ...
    dl.len = 0;
    try field.compose(gpa, &f, particles.slice(), light, cell_w, cell_h, &dl);
    _ = try feed_view.layoutLoadout(gpa, &engine, @intCast(W), @intCast(H), &dl, null, lens_socket.seatedAccent(feed_t), 0, 0, null, feed_t, .{}, &fh, reply_t, .{}, &rh, zone_t, .{}, &zh, false, false, null, &.{}, "", false, .{ .step = .landing, .answers = .{}, .config = discover.DEFAULT_CONFIG, .name = "", .color = 0 }, .{}, .{ .cards = &.{}, .text = "", .seated = 0 });
    try raster.paint(gpa, &engine, dl.slice(), &fb, clear);
