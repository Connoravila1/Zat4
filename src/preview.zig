...
pub fn main(init: std.process.Init) !void {
    ...
    // items — mine can be unsent) and on theirs (three).
    {
        const menu_rows = [_]chat_view.BubbleRow{
            ...
            .{ .body = "yes — 7pm", .age = "1m", .mine = true, .stamp = false, .kind = .text, .tail = true, .msg = 1 },
        };
        ...
        for (menus) |mc| {
            ...
            dl.len = 0;
            try field.compose(gpa, &f, particles.slice(), light, cell_w, cell_h, &dl);
            _ = try feed_view.layoutChat(gpa, &engine, 430, @intCast(H), &dl, null, feed_view.accent_house, 0, false, false, null, &.{}, &menu_rows, &.{}, 0, "maya.zat4.com", "", .{}, false, false, "", "", .{}, .{}, &.{}, .{}, .{ .top = 40 }, .{}, .confirmed, .connected, .{}, mc.m);
            try raster.paint(gpa, &engine, dl.slice(), &fb, clear);
