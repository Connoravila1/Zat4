...
//!
//! F1 standing record: this project carries exactly TWO third-party imports,
//! both vendored, both with their justification recorded at the wiring site:
//!
//!   1. vendor/stb_truetype.h — public domain, single file, zero transitive
//!      deps. The glyph rasterizer the GUI roadmap §4 pre-justified for
//!      Option C. Full note at the import site (vendor/stb_impl.c).
//!   2. vendor/opus — BSD-3, upstream 1.5.2. The call's speech codec, ruled in
//!      by ZAT_CHAT_CALLING_ROADMAP settled decision #9 as the one audio
//!      dependency, on the same "do not write your own crypto" logic. Full
//!      note at `addOpus` below.
//!
//! Everything else is Zig std; any further import must carry its own written
//! justification before it passes review.
...
pub fn build(b: *std.Build) void {
    ...
    addOpus(b, exe_mod); // the call's audio codec (F1 justification at addOpus)
    ...
    if (ndk_path) |ndk| {
        ...
        addOpus(b, libzat_mod); // the call's audio codec on the phone
...
/// F1 JUSTIFICATION — the vendored Opus codec (`vendor/opus`, BSD-3, upstream
/// 1.5.2, sha256 65c1d2f78b9f2fb20082c38cbe47c951ad5839345876e46941612ee87f9a7ce1
/// verified against both downloads.xiph.org and the GitHub release).
///
/// WHAT IT DOES: compresses 48 kHz voice to 24–48 kbps. The call previously
/// shipped raw S16 PCM — ~810 kbps with headers — which pins the cellular radio
/// in continuous reception for the whole call and is the single largest battery
/// cost in the stack (ZAT_CHAT_CALLING_ROADMAP §5).
///
/// WHY WE WILL NOT WRITE IT OURSELVES: a speech codec is the same class of
/// specialist work as a cipher. Opus is the IETF standard (RFC 6716), is what
/// every serious voice app ships, and is the codec our own `core/rtp.zig`
/// payload convention already names. Settled decision #9 rules it in as the one
/// audio dependency, alongside "use the hardware video encoder."
///
/// COST TO REMOVE: the codec sits behind `shell/opus_codec.zig`, a sealed
/// interface of encoder/decoder open-close-process (D1). Swapping it means
/// reimplementing that file against another codec; nothing above it knows Opus
/// exists beyond the RTP payload type.
///
/// WHY VENDORED RATHER THAN FETCHED: the project builds hermetically and
/// offline — the same posture as the vendored font rasterizer. `deploy/
/// dist-android.sh` and the Windows/macOS cross-compile guards keep working with
/// no network and no -dev packages. Only what a build needs was copied: no
/// autotools, no tests, no demos, no `dnn/` (deep PLC and DRED are opt-in
/// upstream and we do not enable them).
///
/// The source list is upstream's own `celt_sources.mk` / `silk_sources.mk` /
/// `opus_sources.mk` for a FLOAT build, verbatim — every target compiles the
/// same generic C. SIMD (SSE on x86, NEON on aarch64) is deliberately NOT
/// enabled yet: it is a measured optimization (G2), and the arch directories are
/// vendored so switching it on is a change to this function alone.
fn addOpus(b: *std.Build, mod: *std.Build.Module) void {
    // The codec is only reachable through the call stack, which speaks raw
    // Linux sockets and is stubbed out on Windows/macOS. Compiling 137 C files
    // that nothing can call would just slow the cross-compile guards down, so
    // the gate matches the call stack's own (`shell/tui.zig`). Android is
    // `.linux`, so the phone build gets the codec.
    if (mod.resolved_target) |t| if (t.result.os.tag != .linux) return;

    mod.addIncludePath(b.path("vendor/opus/include"));
    mod.addIncludePath(b.path("vendor/opus/celt"));
    mod.addIncludePath(b.path("vendor/opus/silk"));
    mod.addIncludePath(b.path("vendor/opus/silk/float"));
    mod.addIncludePath(b.path("vendor/opus/src"));
    mod.addCSourceFiles(.{
        .root = b.path("vendor/opus"),
        .files = &opus_sources,
        .flags = &.{
            "-std=c99",
            // Required by celt/stack_alloc.h: selects C99 variable-length
            // arrays for Opus's scratch buffers (upstream's default on any
            // C99 compiler; `alloca` is the alternative and is worse).
            "-DVAR_ARRAYS",
            // Tells the public headers they are being compiled as part of the
            // library rather than consumed by a caller.
            "-DOPUS_BUILD",
            "-DHAVE_LRINT=1",
            "-DHAVE_LRINTF=1",
            // Opus's reference C leans on well-defined wraparound in a few
            // fixed-point helpers that are compiled even in a float build;
            // upstream ships with the sanitizer off for the same reason stb
            // does above.
...
/// Upstream's float-build source list (celt + silk + silk/float + src), taken
/// verbatim from the `*_sources.mk` files shipped in the release tarball. Kept
/// beside `addOpus` so a version bump is a mechanical re-copy of one list.
const opus_sources = [_][]const u8{
    ...
    "src/opus_encoder.c",                "src/extensions.c",
    ...
    "src/mapping_matrix.c",              "src/analysis.c",
