...
/// `text`/`created_at` are its content; `reply` carries the thread linkage
/// (null when not a reply) so replay restores it. A disabled store is a silent
/// no-op. `arena` is transient stringify scratch. `reply` is left null on the
/// wire (the encoder omits null optionals) for ordinary posts.
pub fn appendPost(
    store: *Store,
    arena: Allocator,
    author_did: []const u8,
    ...
    const env: PostEnvelope = .{
        .did = author_did,
        .commit = .{ .rkey = rkey, .cid = cid, .record = .{ .text = text, .createdAt = created_at, .reply = reply, .facets = facets, .embed = embed, .tags = tags } },
    };
...
test "store: append then replay rebuilds the index (posts, follows, likes)" {
    ...
    // Write a session's worth of records to the durable log.
    {
        ...
        appendPost(&store, arena, "did:plc:author", "rk1", "bafy-post-1", "hello zat4", "2026-06-14T00:00:00Z", null, null, null, null);
...
test "store: a post's #tag facets survive a restart (replay restores the zone)" {
    ...
    // Append a tagged post to the durable log (as the poll/tap path does).
    {
        ...
        appendPost(&store, arena, "did:plc:author", "rk1", "bafy-post-1", "love #water", "2026-06-14T00:00:00Z", null, &facets, null, &.{"deep"});
    }
...
test "store: a post body with newlines/quotes round-trips as ONE log line" {
    ...
    const tricky = "line one\nline two\r\na \"quoted\" word and a \\ backslash";
    {
        var s = open(path);
        defer close(&s);
        appendPost(&s, arena, "did:plc:a", "rk1", "cidNL", tricky, "2026-06-14T00:00:00Z", null, null, null, null);
    }
...
test "store: replaying the same log twice does not double-apply (idempotent restart)" {
...

    {
        ...
        appendPost(&store, arena, "did:plc:a", "rk1", "bafy-p1", "p", "2026-06-14T00:00:00Z", null, null, null, null);
...
test "store: a disabled store (no path) is a silent no-op, replay of nothing is zero" {
    ...
    defer close(&store);
    try testing.expect(store.fd < 0);
    appendPost(&store, arena_state.allocator(), "did:x", "rk", "cid", "t", "2026-06-14T00:00:00Z", null, null, null, null); // no crash
